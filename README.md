...
<p align="center">
  <img src="https://img.shields.io/badge/版本-v0.9.3-4a6fa5?style=flat-square" alt="version">
...
- 右侧悬浮面板支持单项复制、整组复制、重新生成和页面自动填充；一键填充会跳过已有值的条目，存在活动弹窗时仅填充弹窗表单，并支持原生及 Element UI / Element Plus 的文本框、下拉框、单选、多选、开关与日期时间控件，悬浮球划过后可直接触发。
...
- 可在设置中授权一个父目录，开启后自动创建 `place-fill-data/place-fill-user-data.json`，自动保存并优先加载人工标注；该功能默认关闭。

## 安装

1. 在 [GitHub Releases](https://github.com/coldShan/place-fill/releases) 下载最新的 `place-fill-v0.9.3.zip`。
...
- 开启本地目录自动保存后，授权父目录中的 `place-fill-data/place-fill-user-data.json` 为优先标注来源，同时保留 `chrome.storage.local` 作为运行时兼容存储。
- 所选父目录中已有同名标注文件时，会先询问是否使用当前浏览器标注覆盖；选择取消则保留并使用已有本地文件。
- 关闭本地目录自动保存只会暂停文件读写并保留目录句柄；再次开启时，权限仍有效则直接恢复，需要确认时仅恢复原目录权限，权限已拒绝或句柄丢失时才重新选择目录。
- 首次开启时会直接以固定用途标识申请父目录读写权限；Chrome 提供持久授权选项时，请在当次提示中选择“每次访问都允许”或“始终允许”。浏览器之后要求重新确认时，普通页面会通过悬浮入口主动提醒，点击即可恢复目录权限且无需重新选择目录；该权限仍可由用户随时撤销。
