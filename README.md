...
<p align="center">
  <img src="https://img.shields.io/badge/版本-v0.9.5-4a6fa5?style=flat-square" alt="version">
...
- 右侧悬浮面板支持单项复制、整组复制、重新生成和页面自动填充；一键填充会跳过已有值及密码、验证码、令牌等敏感条目，存在活动弹窗时仅填充弹窗表单，并支持原生及 Element UI / Element Plus 的文本框、下拉框、单选、多选、开关与日期时间控件，无法识别含义的空文本或数值控件会按类型生成随机兜底值，悬浮球划过后可直接触发。
- 智能识别输入框类型，并在输入框旁显示快速填充按钮；当前字段存在常用值时自动展示选择列表，悬浮条目可即时查看备注气泡；五星按钮可添加当前页面已识别的非空字段并跳过敏感信息，黄色状态下确认后可移除对应整组常用数据。
...
1. 在 [GitHub Releases](https://github.com/coldShan/place-fill/releases) 下载最新的 `place-fill-v0.9.5.zip`。
