...
test("dock and smart-fill buttons use icon markup instead of visible text labels", () => {
  ...
  assert.doesNotMatch(smartfillScript, /data-role="smart-fill-recommend-/);
...
test("smart fill menu supports right-click manual annotation and regenerates only the used field", () => {
  ...
  assert.match(smartfillScript, /data-role="smart-fill-add-favorite"/);
  assert.match(smartfillScript, /if \(role === "smart-fill-add-favorite"\) \{\s*Promise\.resolve\(onAddCurrentPageToFavorites\(\)\)/);
  assert.doesNotMatch(smartfillScript, /buildRecommendationItems|openRecommendationPanel|recommendationItems|listRecommendedProfiles/);
  ...
  assert.match(orchestratorScript, /isCurrentPageFavorite:\s*panelController\.isCurrentPageFavorite/);
  assert.doesNotMatch(orchestratorScript, /listRecommendedProfiles:|getCurrentScope:/);
