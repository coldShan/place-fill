(function () {
  ...
  if (!generators || !panelStateApi || !editableTargetApi || !iconAssetsApi || !smartFillApi || typeof document === "undefined") return;

  const fieldKeys = Object.keys(generators.GeneratedFieldLabels);
  ...
  async function copyText(text, opts) {
    ...
    if (navigator.clipboard && typeof navigator.clipboard.writeText === "function") {
      try {
        ...
        hideFallback();
        if (options.flashTone !== null) pulseFlash(options.flashTone || "copy");
        return true;
      } catch (_) {}
    }
    showFallback(text);
    if (options.manualFlashTone !== null) pulseFlash(options.manualFlashTone || "manual");
    return false;
  }
  ...
  function cardTemplate(key, value) {
    ...
    const label = generators.GeneratedFieldLabels[key];
    const iconName = smartFillApi.getFieldIconName(key);
    return [
  ...
  function fillCurrentTarget(fieldKey) {
    const target = editableTargetApi.findEditableTarget(activeSmartTarget) || editableTargetApi.findEditableTarget(document.activeElement);
    const value = state.profile[fieldKey];
  ...
  function getSelectedTargetForImport() {
    return editableTargetApi.findEditableTarget(document.activeElement) || resolveManualOverrideTarget();
  }

  function syncImportedOverrideState() {
    const target = getSelectedTargetForImport();
    if (target) syncSmartButtonForTarget(target);
  }
  ...
  function resolveManualOverrideTarget() {
    ...
    return editableTargetApi.findEditableTarget(document.activeElement);
  }
  ...
  function syncSmartButtonForTarget(target) {
  ...
  document.addEventListener(
    "focusin",
    function (event) {
      syncSmartButtonForTarget(event.target);
    },
    true
  );
  ...
  document.addEventListener(
    ...
    function () {
      window.setTimeout(function () {
        syncSmartButtonForTarget(document.activeElement);
      }, 0);
    },
    true
  );
  ...
  document.addEventListener(
    "contextmenu",
    function (event) {
      lastContextTarget = editableTargetApi.findEditableTarget(event.target);
    },
    true
  );

  window.addEventListener("scroll", function () {
    if (activeSmartTarget && !smartButton.hidden) scheduleSmartButtonPosition();
  });
  ...
  smartButton = document.createElement("div");
  smartButton.className = "ctdp-smartfill";
  smartButton.hidden = true;
  smartButton.setAttribute("data-visible", "false");
  smartButton.setAttribute("data-expanded", "false");
  smartButton.setAttribute("aria-label", "智能填充当前字段");
  document.documentElement.appendChild(smartButton);
  ...
  smartButton.addEventListener("click", function (event) {
    ...
    if (role === "smart-fill-item") {
      ...
      showSmartButton(activeSmartTarget, smartFillApi.inferFieldKeyForSmartFill(activeSmartTarget));
      return;
    }
  ...
  chrome.runtime.onMessage.addListener(function (message) {
    ...
    if (message.type === "apply-smart-fill-override") {
      const target = resolveManualOverrideTarget();
      if (!target) return;
      smartFillApi.setManualFieldOverride(target, message.fieldKey);
      syncSmartButtonForTarget(target);
      return;
    }
